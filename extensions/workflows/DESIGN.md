...
- Stubbed: `budget` = {total: null, spent: () => 0, remaining: () =>
  ...
  with a clear message. Date.now()/Math.random() throw (resume prep —
  same rule as CC).
...
## Persistence (user decision: audit now, architected for resume later)

- RunId: random (persists ⇒ random, per the ID rule; `wf-` prefix).
- `~/.pi/agent/workflows/<runId>/`: script.js, args.json, meta.json,
  workflow.json (status), result.json — atomic writes (temp + rename).
- **Resume prep from day one:** journal.jsonl appends one record per
  agent() call: {seq, promptHash, optsHash, outcome}. Deterministic
  call identity = sequence + hashes. Later resume = serve journal hits
  for the unchanged prefix; requires no format change, only replay
  logic. This is why Date.now()/Math.random() throw in scripts.
- Tool result `details` carries runId so forked sessions can point at
  the artifacts. Stale `running` runs found on disk are normalized to
  `aborted` when listed.

## UI

- /workflows: livePicker of runs (current session + disk, read-only for
  past runs) → detail view: phases, per-agent status/durations, result
  preview. Widget: `▸ workflow <name> running` while active.
- renderCall: meta name/description/phases. renderResult: phase-grouped
  agent tree + result summary.

## Deferred (v2+)

- Resume/replay execution (journal format already in place).
